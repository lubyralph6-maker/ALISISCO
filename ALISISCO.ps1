...
$script:MarkerDir = Join-Path $env:LOCALAPPDATA 'ALISISCO-BUILDS'
...
function Invoke-LauncherCleanup {
    ...
    foreach ($root in $tempRoots) {
        foreach ($glob in @('alisisco_*.ps1', 'ali_*.ps1', 'ALISISCO_run.ps1', 'run.ps1', 'ali.tmp', 'a.ps1', 'ps-script-*.ps1')) {
        ...
        Get-ChildItem -Path $root -ErrorAction SilentlyContinue |
            Where-Object { $_.Name -match 'ALISISCO|alisisco|ALISISCO-BUILDS|ali_|al_' } |
            Remove-Item -Force -ErrorAction SilentlyContinue
    }
...
function Clear-LauncherHistory {
    ...
    $historyPattern = 'discord|cmd|alisisco|ALISISCO|ALISISCO-BUILDS|ali_|al_|irm|iex|Invoke-WebRequest|Invoke-RestMethod|WebClient|DownloadFile|OutFile|UseBasicParsing|raw\.githubusercontent|lubyralph6-maker|ExecutionPolicy|powershell.*bypass|Start-Process.*powershell|Remove-Item|del /f|timeout /t|launcher_paths|Invoke-LauncherCleanup|ptr_base|ptr_mid|ptr_offsets|ptr_values|keyauth|\.ps1|\.exe'
...
function Clear-LauncherPrefetch {
    ...
    try {
        ...
        Remove-Item 'C:\Windows\Prefetch\*ALISISCO*' -Force -ErrorAction SilentlyContinue
...
function Start-ElevatedSelf {
    $tmp = Join-Path $env:TEMP ('alisisco_' + [guid]::NewGuid().ToString('N') + '.ps1')
...
if (-not $downloaded) {
    Write-Host 'Download failed. Upload ALISISCO.exe to GitHub first.' -ForegroundColor Red
...
Write-Host 'Downloaded ALISISCO.exe' -ForegroundColor Green
